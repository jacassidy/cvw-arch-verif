...
ucdbdir = ARCH_VERIF / "fcov_ucdb"
reportdir = ARCH_VERIF / "work"

ucdbs = ucdbdir.rglob("*.ucdb")

# Generate reports
for ucdb in ucdbs:
        ucdbPath = str(ucdb.relative_to(ucdbdir).parent).replace('/', '_')
        cmd = f"vcover report -details {ucdb} -output {reportdir}/report_{ucdbPath}.txt"
        os.system(cmd)

        cmd = f"vcover report -details {ucdb} -below 100 -output {reportdir}/uncovered_{ucdbPath}.txt"
        os.system(cmd)
